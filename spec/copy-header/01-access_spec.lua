...
describe("copy-header", function()
  ...
  describe("requests", function()
    it("does not set auth0-forwarded-for when missing the x-forwarded-for header", function()
      ...
      assert.request(r).has_no.header("auth0-forwarded-for")
    end)

    it("does not set auth0-forwarded-for when private ips are set in x-forwarded-for without a public ip", function()
      local r = assert(client:send {
        method = "GET",
        path = "/request",
        headers = {
    ...
    it("sets auth0-forwarded-for to the client ip in x-forwarded-for", function()
      local r = assert(client:send {
        method = "GET",
        path = "/request", 
        headers = {
      ...
      assert.response(r).has.status(200)
      local header_value = assert.request(r).has.header("auth0-forwarded-for")
      assert.equal("4.3.11.0", header_value)
    end)

    it("sets auth0-forwarded-for to the first public ip from a list of private and public ips in x-forwarded-for", function()
      local r = assert(client:send {
        method = "GET",
        path = "/request",
        headers = {
